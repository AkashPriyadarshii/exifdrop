...
Zero-UI Android app that strips metadata from photos and PDFs the moment you share them. One tap in the share sheet passes a clean file to the real destination. Fully offline. Apache 2.0.
...
Most tools make you open an app, pick the file, strip, then share. ExifDrop does it in the share sheet itself: share from any app → pick ExifDrop → it strips and renames → the file lands clean at your destination. No app screen, no setup, no internet.
...
- **Zero-UI trampoline.** Transparent activity, no layout, never in recents or back stack.
- **Neutral filenames.** `share_<hash>.<ext>` — the `IMG_20250115_140233.jpg` date pattern doesn't survive either.
- **Orientation preserved.** Photos don't share sideways.
- **Cache auto-purge.** Stripped copies older than 24h or over 64MB clear on cold start.
